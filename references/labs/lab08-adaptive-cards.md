...
3. Configure:
   ...
   - Trigger description:
     ```
     This topic helps users request a device when they answer
     yes to requesting one.
     ```
...
2. Use Power Fx to loop through devices:
   ```
   ForAll(Global.VarDevices.value,
          {title: ThisRecord.Model, value: Text(ThisRecord.ID)})
   ```
...
3. Add:
   ```
   - If the user answers yes to requesting a device,
     trigger [Request device].
   ```
4. Save changes
...
3. The agent should:
   ...
   - Ask if user wants to request one
4. Answer `Yes`
...
## Next Steps
Proceed to [Lab 9: Add an Agent Flow for Automation](./lab09-agent-flows.md)
